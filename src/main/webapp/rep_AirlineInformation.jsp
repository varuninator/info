...
try {
			...
			ResultSet result = stmt.executeQuery(str);
			while(result.next()){
				if(result.getString("airline_Id").equals(session.getAttribute("search"))){
					String label = "Airline ID: " + result.getInt("airline_Id");
					%>
					<form method="get" action="rep_AirlineInformation.jsp">
							<label><%=label + " "%><input name = "edit_airline"/></label>
