			...
			if(booked==true){
				...
				if(session.getAttribute("rep_user") != null){
					...
					if(result.getString("username") != null){
						tix = "INSERT otrs.ticket (username, seat_number, first_name, last_name, first_class, business_class, economy_class) value (" + "\"" + session.getAttribute("rep_user") + "\""+", " + (max_seat + 1) + ", \"" + result.getString("first_name") + "\""+", " +  "\"" + result.getString("last_name") + "\"" + ", false " + ", false" + ", true" + ")";	
						//out.print(tix);
					...
					tix = "INSERT otrs.ticket (username, seat_number, first_name, last_name, first_class, business_class, economy_class) value (" + "\"" + session.getAttribute("user") + "\""+", " + (result.getInt("seat_number") + 1) + ", \"" + session.getAttribute("first") + "\""+", " +  "\"" + session.getAttribute("last") + "\"" + ", false " + ", false" + ", true" + ")";	
					//out.print(tix);
					  ps = con.prepareStatement(tix);
					 ps.executeUpdate();
